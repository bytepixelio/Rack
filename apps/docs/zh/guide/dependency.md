...
如果其中一个 range 已经被其它所有 range 包含, 直接返回这个最窄的 range:

```json
vue: "^3.4.0" + vue: "^3.3.0" → "^3.4.0"
```

否则把所有 range 按 `npm` 的 AND 语法拼接, 让包管理器同时强制每一条约束:

```json
foo: "^1.0.0" + foo: "<1.5.0" → "^1.0.0 <1.5.0"
```

::: tip 为什么不直接返回较宽的 range?
单独返回 `^1.0.0` 会让包管理器装 `1.9.x`, 违反 `<1.5.0` 的上限约束。
拼接形式 `^1.0.0 <1.5.0` 是 `npm` / `pnpm` / `yarn` 都支持的语法,
等价于"两个 range 的交集"。
:::
