...
| 选项                        | 说明                                                                              |
| --------------------------- | --------------------------------------------------------------------------------- |
| `-t, --template <template>` | **必填**。要使用的模板 ID, 例如 `@presets/node` 或单个 Registry                   |
| `-n, --name <project-name>` | 预先指定项目名称, 必须是单段安全名 (省略时会进入交互式询问; 用 `.` 表示当前目录)  |
| `-f, --force`               | 允许在已存在的目标目录中初始化 (不会清空目录, 旧文件按各 Registry 的合并策略处理) |
| `--ci`                      | 在 `CI` 模式下运行 (非交互式; 同时跳过依赖安装与 Git 初始化; 必须显式提供 `-n`)   |
...
- **命名空间格式**: 必须以 `@` 开头; 内置的 `@rack` 与 `@presets` 命名空间禁止删除
