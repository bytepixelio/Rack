...
### 1.8 Pipeline 原子性没有覆盖 `package.json` / `rack.json` ✅ 已解决（PR #91 / `f6edecd`）
...
### 6.3 仍有一批用户可复制的文档示例指向不存在的 registry/preset
...
### 6.5 Pipeline 原子性修复仍只是预检，不是完整事务
...
3. ~~前置 `package.json` 损坏预检，避免最常见半安装~~（1.8 已解决，走"预检前置"分支：apply 前先 `pkg.read` 验证 `package.json` 可解析；5.1 已解决，引入显式 `InstallPlan` 收敛角色）。完整事务边界仍见 6.5。
