...
| 6.1 `pnpm lint` 扫描 VitePress 缓存                    | ✅ 已解决   | `28e981a`          | `eslint.config.js` `ignores` 补 `**/.vitepress/cache/**`、`**/.vitepress/dist/**`、`**/coverage/**`，与 `.prettierignore` 的构建/测试产物排除策略对齐；CI 顺序 build → typecheck → lint 通过                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 6.2 Preset 同 canonical 不同版本/语言被静默去重        | ✅ 已解决   | `e910fe1`          | 新增 `DuplicateRegistryError`（`DUPLICATE_REGISTRY`），`buildInstallPlan` 在 BFS 之前对 `requested` 按 canonical key 分组，>1 命中即抛；error-hints 同步加 hint；新增 4 条 install-plan 单测覆盖版本、语言、字面重复、不同标识符 canonical 等价                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 6.3 文档示例指向不存在的 registry/preset               | ✅ 已解决   | `4e3d1dd`          | `apps/cli/README.md`、zh/en `reference/cli.md`、zh/en `guide/preset.md` 的可复制命令替换为真实 `@presets/node` / `@presets/node-library` / `@rack/quality/eslint` / `@rack/testing/vitest`；`--ci` 统一补 `-n <name>`；`language-variants.md` 去掉"官方 Vue.js 变体"说法，改为"假设你已发布"；`priority.md` / `dependency.md` / `namespace.md` 顶端加示例性质 disclaimer                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 6.4 官方 registry 缺自动同步到 R2 链路                 | ⏭️ 不修复   | —                  | 不修复（用户决议）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 6.5 §1.8 措辞误读为完整事务                            | ✅ 已解决   | `5ca68a3`          | §1.8 标题与解决记录表都改成 🟡 部分缓解；结论段"优先修"列表移除 6.5，改成"措辞收准 6.5"；§1.8 说明追加"完整原子性作为低频残余风险记录在 6.5，未实现"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 6.6 CLI runtime help / 测试 / dist 锁死不可运行示例    | ✅ 已解决   | `6dcbe81`          | `overview.ts` / `add/help.ts` / `init/help.ts` 示例换成真实 `@presets/node` / `@rack/quality/eslint` / `@rack/runtimes/node:ts` 等；`add/index.ts` Commander argument 描述同步；`help.test.ts` 断言改为新示例并追加 `DUPLICATE_REGISTRY`；`pnpm --filter rackjs-cli build` 后 `dist/bin.js` 不再包含旧示例                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 6.7 Worker 缺全局异常 JSON 兜底                        | ✅ 已解决   | `b6d82e8`          | `apps/registry-worker/src/index.ts` `fetch()` 包 `try/catch` 包住 `dispatch`，未捕获异常翻译为 `INTERNAL_SERVER_ERROR` 500 JSON + `Cache-Control: no-store`，与 Fastify error-handler 协议对齐；HEAD 沿现有 post-dispatch 路径继续 strip body；新增 4 条 worker 单测覆盖 R2 reject、`versions.json` 解析失败、HEAD 500 body strip                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 6.8 `rk init --name` 路径逃逸                          | ✅ 已解决   | `864f53f`          | 新增 `validateProjectName(name)`：允许 `.` 作为"init 当前目录"特殊值，拒绝空、`..`、含 `/` `\`、`path.isAbsolute(name)`；在 `path.resolve(cwd, projectName)` 之前调用；`init/help.ts` 与 zh/en `cli.md` 同步描述；新增 5 条单测覆盖 `..` / `../outside` / `/tmp/outside` / `a/b` / `a\b` / 空段，`-n .` 保留特殊路径                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 6.9 `version.ts` 找不到 source-tree `package.json`     | ✅ 已解决   | `7a75941`          | `PACKAGE_PATHS` 的 dev entry 从 `'../../package.json'` 改成 `'../../../package.json'`，匹配真实 `src/lib/utils/version.ts` layout；注释路径同步；`version.test.ts` 改为读取 apps/cli/package.json 实际值断言，避免之前 `toMatch(/^\d+\.\d+\.\d+/)` 在 fallback `0.0.0` 下也通过的假阳性                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 6.10 `rack.json.items` 丢失解析后版本                  | ✅ 已解决   | `3453710`          | `ResolvedRegistryItem` 新增 `resolvedIdentifier`（始终携带服务端返回的 `@version`，保留显式 `:language`）；`buildInstallPlan` `toRecord` 改写 `resolvedIdentifier`；`rk add` / `resolveRegistryDependencies` 版本对齐改为非对称：已装 pinned + 请求 unpinned → 视作匹配（manifest 权威），已装 unpinned + 请求 pinned → 保守 VERSION_MISMATCH（旧 manifest 迁移）。`mocks.createItem` 按 production 规则自动派生 `resolvedIdentifier`；install-plan / pipeline / client / e2e `materials` `pipeline` `presets` 测试同步                                                                                                                                                                                                                                                                                                            |
| 6.11 e2e discover 不支持 SemVer build metadata         | ✅ 已解决   | `c33fd8d`          | `apps/e2e/src/discover.ts` 引入 `@rack/registry-core` 的 `SEMVER_PATTERN`，删除私有正则；`apps/e2e/package.json` 加 `@rack/registry-core` workspace 依赖；新增 `tests/discover.test.ts` 4 条 fixture 单测（plain / prerelease / `+build.42` / 非 semver 目录）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 6.12 Server `debug`/`trace` 日志记录认证头             | ⏭️ 不修复   | —                  | 不修复（用户决议）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
